...
export default function () {
  ...
  return (
    <div
      className="w-[1200px]  max-h-[65vh] max-w-[90vw]  p-4 bg-gray-50 rounded-md fixed left-[50%] top-[50%] transform -translate-x-[50%] -translate-y-[50%] overflow-y-scroll"
      onContextMenu={onRightClick}
      ref={containerRef}>
      {/* tabs */}
      <div className="mb-4 p-1 px-2 grid grid-cols-2 gap-2 bg-gray-200 text-gray-400 rounded-sm max-w-max items-center relative">
        <span className="z-10">全部</span>
        <span className="z-10">喜欢</span>
        <span
          className={clsx(
            "absolute left-[4px] w-[36px] bg-green-50 h-[22px] transition-all"
          )}></span>
      </div>
      <div className="grid grid-cols-3 gap-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-gray-50 text-sm">
                    {likeList.current?.includes(urlbase) ? (
                      <FcLike onClick={() => onReverseLikeWallpaper(urlbase)} />
                    ) : (
                      <FcLikePlaceholder
        ...
        {hadMoreWallpaper ?? (
          <div className="h-[214px] flex justify-center items-center">more</div>
        )}
      </div>
    </div>
  )
